...
### Afficher des icônes dans les paramètres d'attribut (ID-10702) 
...
### La découverte JDisc nécessite une adresse IP pour la correspondance 
...
### Le compteur saute un numéro lors de la duplication d'objets (ID-10763) 
...
### Impossible d'accéder au décalage de type chaîne sur une chaîne lors de l'importation depuis JDisc (ID-10699) 
...
### L'API utilise une requête de rapport de la base de données au lieu de créer la nouvelle requête qui conduit à des résultats incorrects (API-496) 
...
### Aucune catégorie dans le résultat lors de l'utilisation de cmdb.objects.read avec les catégories clés (API-505) 
...
### Trop de connexions lors de l'utilisation de requêtes groupées (API-509) 
...
## Module complémentaire de formulaire 1.2.0 
...
## Module complémentaire de flux de travail 1.2 
