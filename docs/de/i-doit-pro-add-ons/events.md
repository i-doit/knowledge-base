Das Event [Add-on](/display/de/i-doit+pro+Add-ons) erlaubt einen hohen Automatisierungsgrad bei der Arbeit mit der [IT-Dokumentation](/display/de/Glossar). Wird etwas an der IT-Dokumentation geändert, können Dritt-Systeme über ebendiese Änderungen informiert werden. Wird beispielsweise eine neue VM in i-doit dokumentiert, kann diese automatisch auf einem Virtualisierungs-Host erstellt und provisioniert werden.

  

  

Ein Praxisbeispiel finden Sie auf unserem [Blog](https://www.i-doit.com/blog/event-add-on/)

**Inhaltsverzeichnis**

*   1[Konfiguration](#Events-Konfiguration)
*   2[Logging](#Events-Logging)
*   3[Releases](#Events-Releases)

Rechtevergabe

Unter `**Verwaltung → Rechtesystem → Rechtevergabe → Events.**` können [Rechte für Personen und Personengruppen](https://kb.i-doit.com/display/de/Rechteverwaltung) angepasst werden.

![](/download/attachments/34242583/image2022-3-16_16-17-17.png?version=1&modificationDate=1647443837856&api=v2&effects=drop-shadow)
...
![](/download/attachments/34242583/events.png?version=2&modificationDate=1461676295700&api=v2&effects=drop-shadow)

404 Not Found

Erscheint beim Aufruf der Event-Konfiguration lediglich eine Fehlermeldung, die Seite könne nicht gefunden werden, liegt das mit aller Wahrscheinlichkeit an einer fehlerhaften [Konfiguration des Webservers](/display/de/Systemeinstellungen). Sowohl das Apache-Modul `rewrite` muss aktiviert sein, als auch das Einlesen der `.htaccess`\-Datei im Installationsverzeichnis von i-doit muss erlaubt sein (`AllowOverride All`).
...
*   [Kategorie](/display/de/Glossar)
    *   Neuanlage (nur über die Web GUI)
    *   Speichern
    *   Archivieren/Löschen/Wiederherstellen/Purge
*   [Objekt](/display/de/Glossar)
    *   Neuanlage
    *   Purge
*   [Objekttyp](/display/de/Glossar)
    *   Neuanlage/Speichern
    *   Purge

Es existieren demnach für alle [Zustände in der IT-Dokumentation](/display/de/Lebens-+und+Dokumentationszyklus) passende Events. Es sind beliebig viele Event-zu-Befehlsaufruf-Kombinationen möglich.
...
![](/download/attachments/34242583/event_konfiguration.png?version=2&modificationDate=1461676295012&api=v2&effects=drop-shadow)
...
Die beschriebenen Shellscripte sind nicht nur dafür geeignet, Dritt-Systeme zu steuern, sondern auch i-doit selbst. Es spricht nichts dagegen, über ein solches Shellscript das Kommandozeilen-Tool von i-doit, den [Controller](/display/de/CLI), oder die [API](/pages/viewpage.action?pageId=7831613) aufzurufen. Dadurch können automatisierte Arbeiten innerhalb der IT-Dokumentation erledigt werden.
...
![](/download/attachments/34242583/event_log.png?version=1&modificationDate=1461676295452&api=v2&effects=drop-shadow)
...
| 1.1.1 | 31.07.2019 | ```<br>[Bug] Drop-down empty in hooks for object types<br>``` |
| 1.1 | 30.01.2019 | ```<br>[Bug] Constant of user defined categories is missing<br>``` |
