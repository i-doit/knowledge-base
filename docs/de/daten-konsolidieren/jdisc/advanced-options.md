...
| **Matching auf zugewiesene Objekttypen begrenzen** | Begrenzt die Suche nach einem bereits vorhandenen Objekt auf die i-doit Objekttypen, die in der Zuordnungstabelle (oben) verwendet werden. Ist die Option deaktiviert, kann ein Gerät auch einem Objekt eines beliebigen anderen Objekttyps zugeordnet werden, sobald dieses die Kriterien des Import Matching Profils erfüllt, etwa ein Objekt mit derselben Objekt-Bezeichnung. |
