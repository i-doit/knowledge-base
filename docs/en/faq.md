...
Upon purchase, you receive a **license token**. You use this token to log in to the [customer portal](administration/kundenportal.md) — and the same token is also used for license activation in i-doit.
...
Details: [Licensing](wartung-und-betrieb/lizenzierung.md)

### The license is valid, but i-doit shows "not licensed"?

Common causes:

- **Firewall blocks license verification** — i-doit must be able to reach `lizenzen.i-doit.com` and `center.i-doit.com` on port 443. See [Firewall rules](wartung-und-betrieb/sicherheit-und-schutz.md#firewall-freigaben-fur-i-doit).
...
Current requirements for operating system, PHP, MariaDB, and Apache can be found under [System requirements](installation/systemvoraussetzungen.md).

### How do I update i-doit?

Two ways: Via the **web interface** (with internet access) or manually via the **console** (upload update package). The complete guide: [Applying updates](wartung-und-betrieb/update-einspielen.md)

### Can I skip versions?

Partially. From version 20 onwards, you can skip versions, but there are defined **intermediate steps** — for example, you cannot jump directly from version 20 to 38. Which version jumps are possible is described in the [update guide](upgrades-und-umzuege/update-anleitung.md). If in doubt: Download the latest update package from the [customer portal](administration/kundenportal.md) — the update wizard will show you the next possible step.
...
4. **Check known issues** — [Known update issues](administration/troubleshooting/bekannte-update-probleme.md)

### Which PHP version do I need?

See [System requirements](installation/systemvoraussetzungen.md). If you need to update PHP: [Updating PHP](wartung-und-betrieb/php-update.md)
...
Export the Excel spreadsheet as CSV and use the [CSV data import](daten-konsolidieren/csv-datenimport/index.md). First create an import profile that maps the columns of your CSV file to the i-doit categories.
...
JDisc Discovery scans your network and delivers hardware and software data to i-doit. The setup: [JDisc Discovery](daten-konsolidieren/jdisc/index.md)

### Can I automate the import?

Yes, via [cronjobs](wartung-und-betrieb/cronjobs-einrichten.md). Both CSV and JDisc imports can be scheduled via `console.php`. For complex commands, [configuration files](automatisierung-und-integration/cli/configuration-files.md) are recommended.
...
Via the [LDAP synchronization](benutzerauthentifizierung-und-verwaltung/ldap-verzeichnis/index.md). You configure an LDAP server in the Administration and set up a cronjob for regular synchronization. A complete configuration example can be found under [Configuration files — ldap-sync](automatisierung-und-integration/cli/configuration-files.md#beispiel-fur-den-befehl-ldap-sync).
...
1. **Check system:** Enough RAM? CPU utilization? Disk full? See [System requirements](installation/systemvoraussetzungen.md).
...
6. **Set up cronjobs:** Regular [maintenance cronjobs](wartung-und-betrieb/cronjobs-einrichten.md) keep the system clean.
...
Yes — via shell script with `mysqldump` + `rsync` or via `system:tenant-export` / `system:tenant-import`. A step-by-step guide can be found under [Prod→Test database synchronization](anwendungsfaelle/prod-test-datenbank-synchronisation.md).
...
- [API v1 methods](i-doit-add-ons/api/methoden/v1/index.md) — Complete reference
- [API v2 methods](i-doit-add-ons/api/methoden/v2/index.md) — New API version
- [Examples for using the API](i-doit-add-ons/api/beispiele-zur-nutzung-der-api.md) — Practical examples

### How do I secure the API?

Keep the API key secret, restrict access by IP, create a dedicated API user with minimal permissions. Details: [Securing the API](wartung-und-betrieb/sicherheit-und-schutz.md#api-absichern)
...
With the [Report Manager](auswertungen/report-manager.md). You can create reports using the visual query editor or write SQL directly. Reports can also be [exported via cronjob](wartung-und-betrieb/cronjobs-einrichten.md#report-export).

### Where do I find the changelogs?

Under [Changelogs](versionshistorie/changelogs/index.md) — all changes per version are documented there. The most important new features per major version can be found in the [Release Notes](versionshistorie/release-notes/index.md).
