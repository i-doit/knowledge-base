...
New PHP versions bring security fixes, better performance, and new features. At the same time, i-doit requires specific PHP versions -- always check the [system requirements](../installation/systemvoraussetzungen.md) before updating to see which versions are currently supported.
...
- **Missing modules**: `php -m` shows which modules are loaded. Compare with the [system requirements](../installation/systemvoraussetzungen.md)
