# Install i-doit-Discovery for Windows
...
The installation package will always include the latest [i-doit version](../../../version-history/index.md), as well as latest JDisc Discovery version with the respective add-ons included. Also the `src.zip` folder allows for an installation without internet connection.<br>
...
[![GUI](../../../assets/images/en/installation/manual-installation/microsoft-windows-server/i-doit-discovery-windows/1-ijdw.png)](../../../assets/images/en/installation/manual-installation/microsoft-windows-server/i-doit-discovery-windows/1-ijdw.png)
...
The following instructions show you how to set up SSL for Windows with i-doit-Discovery.
Only the steps required to configure HTTPS are described.<br>
This segment can be skipped.

### Prerequisites

First you need a valid certificate in `.crt` and `.key` format. You can create these files with OpenSSL.<br>
You can download and install OpenSSL here: [OpenSSL](https://slproweb.com/products/Win32OpenSSL.html)<br>
Once OpenSSL is installed, you can open the Win64 OpenSSL Command Prompt via the Windows search bar by searching for 'OpenSSL'.
...
The certificate and the private key have now been created in the folder in which the command was executed. Copy these files to the folder `i-doit-Discovery\apache-2.4\conf\extra\`, for example.

### Configuration steps

1. **Creating the ssl.conf file**<br>

Navigate to your i-doit-Discovery folder under `i-doit-Discovery\apache-2.4\conf\extra\` and create the file `ssl.conf`. The file should have the following content:
...
**Note:** Customize the paths and configuration settings according to your own environment and make sure that your certificate and private key are present in the specified paths.

2. **Adjustments in the httpd.conf**<br>

Edit the `httpd.conf` file, which is located under `i-doit-Discovery\apache-2.4\conf\`:
...
The Apache web server has now been restarted, check the installation and whether i-doit Discovery is accessible via HTTPS.

That was it! Your i-doit Discovery installation is now configured for SSL on Windows.
...
To remove JDisc and the JDisc add-ons you can use ==Add or Remove Programs== as well.
