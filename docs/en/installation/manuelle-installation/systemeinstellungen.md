...
For operating system configuration, we recommend a set of settings related to PHP, Apache Web Server, and MySQL/MariaDB. These are based on the [system requirements](../systemvoraussetzungen.md) for i-doit. The settings differ depending on the operating system, which is why we refer to the respective articles:
