...
The [IT documentation](../../glossary.md) contains business-critical data -- network diagrams, contract information, credentials, and configurations. Data loss due to hardware failure, human error, or an attack can have serious consequences. That is why i-doit belongs in every backup strategy.
...
Also disable [cronjobs](../set-up-cron-jobs.md) during the backup.
...
- **Regularity** -- back up daily, at minimum before every [update](../i-doit-update.md)
...
For a complete migration to another server, read the article [Migrating an installation on Linux](../../upgrades-and-migrations/migration-on-linux.md).
