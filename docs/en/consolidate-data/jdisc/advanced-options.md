...
| **Restrict matching to assigned object types** | Limits the search for an existing object to the i-doit object types used in the assignment table (above). With the option disabled, a device can also be matched to an object of any other object type as soon as it fulfils the criteria of the import matching profile, for example an object with the same object title. |
