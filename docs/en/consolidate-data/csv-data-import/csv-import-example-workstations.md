...
This example builds on the [CSV import of locations](csv-import-example-locations.md). You import workstations, assign them to a room, and link them with clients, monitors, and printers.
...
This import has dependencies: the locations must already exist (see [Creating Locations](csv-import-example-locations.md)). Additionally, the workstations must be created **before** the components so that clients, monitors, and printers can be directly assigned. Therefore, the workstations appear first in the CSV file.
