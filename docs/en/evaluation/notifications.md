...
| **Threshold**                               | The trigger for the notification. Depending on the type, this is a **time interval** (e.g., 30 days) or a **count** (e.g., 5 licenses). A detailed explanation is displayed directly in the form. If a exact match is necessary is configured via [Notify only for exact match of threshold](../system-administration/administration/tenant-management/settings-for-tenant.md#notifications). |
