...
Reporting is a suitable tool for condensing, preparing, and sharing data with third parties. _i-doit_ therefore includes a Report Manager that can fully process data from the [IT documentation](../glossar.md).
...
With the Report Special add-on, approximately 50 pre-built reports are imported. More information about the add-on can be found in the [Customer Portal](../administration/kundenportal.md).
...
Existing reports can be automatically exported as files without manual user intervention. This is done via the [i-doit console utility](../automatisierung-und-integration/cli/index.md) using `report-export`.
...
!!! info "Report-based notification"
    _i-doit_ can send appropriate [emails](benachrichtigungen.md) for positive reports, i.e., reports with results.
...
With [Variable Reports](../anwendungsfaelle/variable-reports.md), you can display data that is object-specific, rather than static as with normal reports.
