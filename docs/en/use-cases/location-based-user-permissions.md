...
Mr. Mueller is responsible for all servers at the Duesseldorf location. Mr. Mueller's rights should be restricted so that he can see all servers at the Duesseldorf location and create new server [objects](../basics/it-documentation-structure.md). For all servers at the Duesseldorf location, he should be able to edit all [categories](../basics/it-documentation-structure.md). He should not have access to servers or other objects at other locations.

## Configuration

To enable read and edit access to the objects and their categories, the rights **View** and **Edit** are set under **[Administration > Settings for [Tenant Name] > Permissions > CMDB](../efficient-documentation/permission-management/cmdb.md)** for Mr. Mueller's **Person** object with the **conditions**
...
For these restrictions to also take effect in the [object browser](../basics/object-relationships.md), in the [location view](../use-cases/locations.md), and in the [CMDB Explorer](../evaluation/cmdb-explorer/index.md), advanced settings must be configured under **Administration > [Tenant Name] Management > Expert Settings**:
...
A detailed explanation of these settings can be found in the article [CMDB (Rights Management)](../efficient-documentation/permission-management/cmdb.md).
