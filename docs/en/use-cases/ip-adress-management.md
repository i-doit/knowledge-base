...
IP Address Management (IPAM) -- managing IP networks, addresses, and network components -- is an essential element of your [IT documentation](../glossary.md).

## Overview

i-doit comes with all [object types](../glossary.md) and [categories](../glossary.md) needed to cover [IPAM](https://en.wikipedia.org/wiki/IP_address_management) in the standard installation. These include:
...
Before documenting individual IP addresses, hostnames, and MAC addresses, it is worthwhile to document the corresponding LANs (Local Area Networks). The **Layer 3 Net** object type is suitable for this purpose. The **Net** category contains the essential [attributes](../glossary.md) for maintaining an IP network:
