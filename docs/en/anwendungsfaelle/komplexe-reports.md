...
The required knowledge about [reporting](../auswertungen/report-manager.md) and the [database structure of i-doit](../software-entwicklung/datenbank-modell/index.md) can already be found in the corresponding articles. Good knowledge of SQL and the SQL functions provided by MySQL/MariaDB is clearly an advantage.

Time Intervals
--------------

A common requirement for a report is to get information about a specific time period. i-doit already provides the necessary fields with date entries, for example when an [object](../grundlagen/struktur-it-dokumentation.md) was last modified or when a maintenance contract expires. The query editor in the Report Manager can address these fields, but only with fixed date values: Which objects have been modified since 01/01/2016? However, the date should often be relative to the current time: Which objects have been modified this month? This does not specify which month is meant, but rather assumes the month in which the report is executed.

To generate such a report, we first create a new report with the query editor. For output, we add for example [object title](../grundlagen/eindeutige-referenzierungen.md), [object type](../grundlagen/struktur-it-dokumentation.md), date of last change, and the name of the editing person.
