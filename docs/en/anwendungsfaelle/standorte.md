...
i-doit already comes with suitable [object types](../grundlagen/struktur-it-dokumentation.md) in the standard installation that can be used (or hidden) for documenting locations:
...
In addition to these object types, each object has [categories](../grundlagen/struktur-it-dokumentation.md) where the placement is recorded:
...
Server racks can also be used as locations [to position hardware there](../auswertungen/rack-ansicht.md).
...
A documented workstation receives a person as a logical location. In addition, workstations can be assigned to physical locations. The **Location View** (see above) can display either physical or logical locations, or a combination of both. This option is [user-defined](../benutzerauthentifizierung-und-verwaltung/integrierte-authentifizierung/lokalen-benutzer-anlegen.md).
...
For each user or group of users, read, write, and many other [rights for locations and subordinate objects](./standort-basierte-benutzerrechte.md) can be granted.
