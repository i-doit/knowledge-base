...
Multi-tenancy means that you can run multiple completely separate [IT documentations](../glossary.md) with a single i-doit installation. Each tenant operates in its own isolated data set — as if it were a separate i-doit instance, but without the effort of maintaining multiple installations.
...
Not every form of data separation requires its own tenant. Before creating a new tenant, check whether the [permission management](../user-authentication-and-management/built-in-authentication/index.md) within a tenant is sufficient:
...
-   **Remove**: **Irrevocably** delete a tenant — including its database. Can only be restored from a [backup](../maintenance-and-operation/backup-and-restore/index.md).
...
| **Tenant-specific settings** | — | Yes — see [Settings](management/tenant-management/tenant-settings.md) |
...
!!! attention "Updating i-doit"
    During the [i-doit update](../maintenance-and-operation/i-doit-update.md), you are asked which tenant databases should be updated. By default, all are selected. It is strongly recommended to always include all tenant databases in the update. Exceptions apply only in well-justified special cases — for example, on instruction from [support](../administration/troubleshooting/index.md).

## User management

You configure [user management](../basics/initial-login.md) per tenant. Person objects with login data can differ by tenant — a user can be an administrator in tenant A and not exist at all in tenant B.
...
If you connect an [LDAP directory or Active Directory](../user-authentication-and-management/ldap-directory/index.md), i-doit queries the configured servers sequentially during login and then determines for which tenants the credentials are valid.

With active [Single Sign On (SSO)](../user-authentication-and-management/sso-comparison/index.md), you can set a default tenant in the [Admin Center](admin-center.md#system-settings). i-doit then automatically connects the user to this tenant without requiring a selection.

## Switching tenants

You can switch between tenants without logging out first.

In the upper [main navigation bar](../basics/it-documentation-structure.md), you can see which user is logged into which tenant. Hover over the tenant name — a drop-down menu appears with the other available tenants. One click switches you directly there.
...
-   [Settings](management/tenant-management/tenant-settings.md) — tenant-specific configuration
-   [Backup and Restore](../maintenance-and-operation/backup-and-restore/index.md) — backup strategies for multiple tenants
