...
The logbook records all changes in i-doit. You can also find it under **Extras → CMDB → Logbook**. Entries can be filtered by various parameters, and each entry shows -- when possible -- the exact changes (which value was changed from what to what). [READ MORE](../../../grundlagen/logbuch.md)
...
For the archiving to actually take place, you must execute the [logbook:archive](../../../automatisierung-und-integration/cli/index.md) command.
