...
The [object matching profiles](../../../consolidate-data/identify-objects-during-imports.md) were created to define criteria that are used when you want to establish a connection between an external object and an i-doit object via an interface. You can specify how many and which attributes must match for a successful comparison -- for example, hostname, MAC address, serial number, or a combination thereof.
