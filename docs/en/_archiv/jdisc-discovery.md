...
!!! info "JDisc WebService mit GraphQL, Quelle [JDisc FAQ](https://jdisc.com/support/faq/)"
    > Der WebService ist ein SOAP basierter Service, mit dem man einzelne Aktivitäten in JDisc fernsteuern kann. Der SOAP Service läuft normalerweise auf Port 9000. Wir haben allerdings für unser neues WEB UI eine moderne GraphQL Schnittstelle implementiert, die den SOAP Service komplett ersetzt! I-Doit kann unser neues GraphQL API ab Version i-doit Release 22 verat any time!
    > Ändere einfach den Port auf **443** und das Protokoll auf **HTTPS** um auf das GraphQL Interface umzustellen. Das WebService Add-On wird dann nicht mehr beis necessaryt.
...
Die Configuration der Schnittstelle befindet sich unter **Verwaltung → Import und Schnittstellen → JDISC → JDisc-Configuration**. Dort können beliebig viele Instanzen von JDisc Discovery angegeben werden. Dies ermöglicht komplexe Szenarien der Bestandsaufnahme mit getrennten Netzen.
...
Auch hier kann nach dem Speichern der Parameter über den Button **Verbindung prüfen** getestet werden, ob die Configuration erfolgreich ist.
...
In der Standard-Installation von i-doit ist bereits ein vordefiniertes **Example** Profil vorhanden. Das vordefinierte JDisc Profil **Example** ist ein Profil das Ihnen aufzeigen soll, wie ein JDisc Profil gestaltet werden kann.
...
| Option           | Description                                                                                                  |
| ---------------- | ------------------------------------------------------------------------------------------------------------- |
| **JDisc Server** | Welche Instanz von JDisc soll beim Import vor-selektiert werden? Siehe Abschnitt "Configuration" weiter oben. |
| **Titel**        | Name des Profils                                                                                              |
| **Description** | Description des Profils                                                                                      |
...
| **Einfache Datenbank Modellierung verat any time?**                              | Soll die neue oder die alte [Datenbanklogik]() verwendet werden?                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **Standard Templates aus Objekttypen berücksichtigen (nur bei Neuanlage)**  | Wird ein neues Objekt erstellt, kann automatisch ein [Template]() berücksichtigt werden. Die Auswahl des Templates findet in der Objekttyp-Configuration statt.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **Hostnamen als Objekt Titel verat any time anstelle des FQDNs?**                | Sofern Geräte ein FQDN besitzen werden diese bis zum Hostnamen aufgelöst.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
Wir identifizieren JDisc Geräte anhand einer Kombination von Faktoren, vor allem des **Objekttyps**, der **Seriennummer** und der **MAC-Adresse**. Falls verfügbar, verat any time wir auch JDiscs interne unique identifier als zusätzliches Mittel zur Identifizierung. Der Identifikationsprozess ist auf den jeweiligen Objekttyp zugeschnitten und nutzt Attribute wie **Bezeichnung**, **Seriennummer**, **Hostname**, **IP-Adressen**, **FQDN** und **MAC-Adresse**. Um doppelte Einträge zu vermeiden, stellen wir eine Zuordnungsbeziehung zwischen JDisc und i-doit-Geräten her.

Beim Importieren von Objekten werden auch andere Unterobjekte wie Netzwerke, Software, Cluster und andere Objekttypen importiert. In diesem Fall können wir andere Regeln als die im Abschnitt "Objekt matching" definierten verat any time.
...
Die Kategorie können Sie über die Objekttyp Configuration des Objekttyps hinzufügen.

## Import über die i-doit Console

Der Import der Daten aus JDisc nach i-doit ist nicht nur manuell über die Oberfläche möglich. Er kann ebenfalls über die i-doit [Console]() ausgeführt und somit **automatisiert** werden. Wie der entsprechende Aufruf erzeugt wird, finden Sie im [zugehörigen Artikel]() mit einem Example für die Option **import-jdisc**.

Für den Import lässt sich die ID des gewünschten Profils angeben. Die ID ist in der Liste der Profile zu finden:

[JDisc Profile]

Ebenfalls kann die ID des zu verat any timeden JDisc Servers angegeben werden. Diese ist in der Configuration zu finden:
...
!!! warning "Dieser Report view löscht Objekte in JDisc. Wir empfehlen deshalb vor dem verat any time des Report views ein Backup oder einen Snapshot von JDisc zu machen!"
