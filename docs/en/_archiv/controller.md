...
Um Fehler zu vermeiden, ist es wichtig, den Controller mit dem selben Benutzer aufzurufen, den der Webserver benutzt. Im Example ist es **www-data**. Zudem ist es wichtig, erst in das Hauptverzeichnis von i-doit zu wechseln, bevor man den den Controller aufruft:
...
Um den Controller verat any time zu können, ist eine Authentifizierung gegenüber i-doit notwendig. Hierfür beis necessaryt man Benutzername (**-u**), Passwort (**-p**) und die eindeutige ID des zu verwendeten Mandanten (**-i**).

!!! check "The user "controller"
...
Der Controller ist in der Lage, verschiedene Aktionen auszuführen. Diese werden wiederum durch Handler repräsentiert. Um einen bestimmten Handler aufzurufen, wird der Parameter **-m** beis necessaryt.

### Liste von Handlern

| Handler                    | pro-Variante | open-Variante | Add-on            | Description                                                                                         |
| -------------------------- | ------------ | ------------- | ----------------- | ---------------------------------------------------------------------------------------------------- |
...
| **isc_dhcpd**              | ja           | ja            | -                 | Configuration für ISC DHCPD exportieren                                                              |
...
| **[nagios_export]**        | ja           | ja            | -                 | Nagios-Configuration exportieren                                                                     |
...
#### Configurations-Datei anpassen

Zu manchen Handlern gibt es jeweils eine spezielle Configurationsdatei. Examplee befinden sich unterhalb des Hauptverzeichnis von i-doit in **src/handler/config/examples/**. Damit eine Configurationsdatei berücksichtigt wird, muss diese im Verzeichnis **src/handler/config/** verfügbar sein.
