...
In addition to the generally applicable [system requirements](../../installation/systemvoraussetzungen.md) and system settings for i-doit, this add-on requires the following conditions to be met on the system where it is installed:
...
_i-doit_ provides a fine-grained [permission management](../../effizientes-dokumentieren/rechteverwaltung/index.md) system, which is also used for VIVA. Permissions can be assigned per user/user group under **Administration → Authorization system → VIVA**. The following permissions are available:
...
*   The catalogs EL 12 from 2011 are to be used in VIVA. The [import](./vorgehensweise-mit-viva.md) must already be completed.
