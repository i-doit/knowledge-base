...
| **Restrict matching to assigned object types**                                     | Limits the search for an existing object to the i-doit object types that the object type assignment of this profile uses (see above). Without this option, a device can also be assigned to an object of a different object type if the object matches the criteria of the [Import Matching Profile](../../administration/management/import-and-interfaces/import-matching-profile.md), for example an object that carries the same object title. |
