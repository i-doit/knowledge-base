...
Check\_MK is a [network monitoring](../automatisierung-und-integration/network-monitoring/index.md) software that you can configure from within i-doit.
...
- **Export assigned contacts**: Stores the contacts assigned to the [objects](../grundlagen/struktur-it-dokumentation.md) including [object title](../grundlagen/eindeutige-referenzierungen.md) in the configuration.
...
Assign the [category folder](../grundlagen/struktur-it-dokumentation.md) **Check_MK (Host)** to the desired [object types](../grundlagen/struktur-it-dokumentation.md) via [Edit data structure](../administration/verwaltung/datenstruktur/datenstruktur-bearbeiten.md).
...
*   **CMDB tags**: tags that are derived from a CMDB [attribute](../grundlagen/struktur-it-dokumentation.md)
...
Via the **Service assignment**, you link installed software with service checks that are read via [Livestatus](../automatisierung-und-integration/network-monitoring/daten-abfragen-mit-livestatus.md). This allows the Analysis module to determine which IT services are directly or indirectly affected based on monitoring outages.
...
1. It controls the [i-doit Console](../automatisierung-und-integration/cli/index.md).
