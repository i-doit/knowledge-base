...
*   **auth/**
    Legacy: Used for all authorization system code. See also [Using User Rights in Add-ons](using-permissions-in-add-ons.md).

*   **example/**
    Legacy: Serves as an example for custom code in "legacy" format.

*   **install/**
    This is where files used for the installation and/or update of an add-on are located (update_data.xml, update_sys.xml, and update_files.xml). See also [Installing, Updating, and Activating Add-ons](install-update-activate-add-ons.md).

*   **lang/**
    This directory contains translation files (de.inc.php and en.inc.php) for German and English.

*   **src/**
    This is the directory for code in PSR-4 format. The add-on namespace (here: idoit\Module\Example) should be mapped to this directory. See also [Routing and MVC](routing-and-mvc.md).
...
*   **init.php**
    The "init.php" file is required to make various preparations for the add-on; for example, autoloaders are registered and constants are defined here. See also [Bootstrapping an Add-on (init.php)](bootstrapping-an-add-on.md).

*   **isys_module_example_autoload.class.php**
    This autoloader class is required as soon as code in "legacy" format is used (e.g., API, authorization system, categories, ...). See also [Bootstrapping an Add-on (init.php)](bootstrapping-an-add-on.md).
...
*   **package.json**
    The "package.json" file is required and contains important metadata about the add-on -- see also [Add-on Metadata (package.json)](add-on-metadata.md).
