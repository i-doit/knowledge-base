---
title: CSV Import Example Workstations
description: This example builds on the CSV import of locations.
icon:
status:
lang: en
---
...
This example builds on the [CSV import of locations](beispiel-fuer-den-csv-import-standorte-erstellen.md). You import workstations, assign them to a room, and link them with clients, monitors, and printers.

!!! note "This article was last verified for i-doit version 1.16.2"

### Required Information

-   Object name -- Name of the workstation, client, monitor, or printer
-   Location -- Under which location should the object be placed?
-   Object type -- Which object type will be created?
-   Workstation assignment -- To which workstation will the component be assigned?

Example CSV file for this import:

[Import-workstations.csv :material-file-download:](../../assets/images/de/daten-konsolidieren/csv-import/csv-arbeitsplaetze/Import-Arbeitsplaetze.csv){ .md-button .md-button--primary }

??? example "Import-workstations.csv"
    ```text
    objectName;Standort;objecttyp;AP Zuweisung
...
This import has dependencies: the locations must already exist (see [Creating Locations](beispiel-fuer-den-csv-import-standorte-erstellen.md)). Additionally, the workstations must be created **before** the components so that clients, monitors, and printers can be directly assigned. Therefore, the workstations appear first in the CSV file.

### Step-by-Step Instructions

1. Navigate to the CSV import under **Extras > Import > CSV Import**
2. Upload the CSV file and click **Prepare for Import**
3. Leave the settings at the default values and click **Prepare Mapping**
...
After the import, the workstations are assigned to a location. Under each workstation, you will find the associated client, monitor, and printer.
