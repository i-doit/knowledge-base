...
The first row serves as a header and identifies the respective [attribute](../../grundlagen/struktur-it-dokumentation.md). Each additional row corresponds to an [object](../../glossar.md) in the IT documentation.
...
If you select a global [object type](../../grundlagen/struktur-it-dokumentation.md) under **Global object type**, i-doit imports all objects from the `.csv` file as this type.

Without a global object type, you need an additional column in the CSV file containing the [database constant](../../grundlagen/struktur-it-dokumentation.md) of the object type. This way you can import different object types (client, printer, monitor, etc.) in a single file. This specification must be present for all objects -- a mixed form is not possible.

### Separator, headers, and default template

If you use a different **separator** than the semicolon (`;`), specify it here. Without a header row, deactivate this option -- i-doit will then treat the first row as an object. Optionally, objects are created with the data from the [default template](../../effizientes-dokumentieren/templates.md).
...
Specify how [list categories](../../grundlagen/struktur-it-dokumentation.md) (multi-value) are represented in the `.csv` file. If your import does not involve such categories, skip this section.

In the example above, "Client 02" appears multiple times: this client has two CPUs, so the list category **CPU** should receive two entries. You achieve this through an additional **row** -- you do not need to repeat unchanged [attributes](../../glossar.md) there.
...
Select an [**object matching profile**](../objekt-identifizieren-bei-importen.md) if you want to update existing objects. You can edit the profile at any time afterwards.
...
*   With a global object type, you only need to assign the [object title](../objekt-identifizieren-bei-importen.md).
...
You can also create [relationships](../../grundlagen/objekt-beziehungen.md) between objects via the CSV import. Place the object to be linked in a separate column and assign the corresponding attribute during mapping. An example for the physical location in column H:
...
In addition to the manual import via the web interface, you can also import CSV files with the [i-doit console utility](../../automatisierung-und-integration/cli/index.md). This allows you to automate regular updates. The required profile IDs can be read with the command `import-csvprofiles`.
