...
The structure of IT documentation describes how i-doit organizes your [IT documentation](../glossar.md). Here you will learn the central terms -- from object type groups and objects to attributes -- and see how they work together in the Web GUI.

## From the navigation

After [logging in](erstanmeldung.md), the [dashboard with widgets](dashboard-und-widgets.md) appears first. From there you explore the IT documentation. Via the main navigation bar (the black area at the top) you can reach almost all functions in i-doit.
...
[Categories are assigned to object types](zurodnung-von-kategorien-zu-objekttypen.md), not to objects. Every object of an object type has the same categories assigned.
...
The "General" category belongs to those categories that are an integral part of every object. Therefore, it cannot be deselected per object type. This category holds important attributes such as the object title, SYS-ID, [CMDB status and condition](lebens-und-dokumentationszyklus.md).
