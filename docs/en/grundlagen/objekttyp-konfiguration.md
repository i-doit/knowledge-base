...
The name of the object type. You can either enter a plain-text name (e.g. `Network Printer`) or use a [language constant](../administration/mehrsprachigkeit-und-uebersetzungen.md) (e.g. `LC__OBJTYPE__NETWORK_PRINTER`). Language constants are recommended if you operate i-doit in multiple languages -- the name is then automatically displayed in the language of the logged-in user.

### SYSID prefix

Determines the beginning of the [SYSID](kategorien/global.md) for all objects of this type. If you leave this field empty, the default prefix `SYSID` is used and a Unix timestamp is appended. With a custom prefix (e.g. `SRV-`), the object ID is appended instead, resulting in more readable SYSIDs: `SRV-42` instead of `SYSID_1680000042`.

### Automatic inventory number

Defines a schema for automatically generated inventory numbers in the [accounting category](kategorien/accounting.md). Placeholders are available (click the blue question mark). Example: `SRV-%COUNTER%` produces `SRV-001`, `SRV-002` etc. Can also be combined with the object title: `%TITLE%-%COUNTER%`.
...
Each object type can have its own color. This color appears in the [CMDB Explorer](../auswertungen/cmdb-explorer/index.md) and in the [rack view](kategorien/s-enclosure.md). Helps with visual distinction -- e.g. servers blue, switches green, clients yellow.
...
Controls the direction of [object relationships](objekt-beziehungen.md). Normally, the object you are editing is the "master" and the linked object is the "slave". If the linked object type is configured as relationship master, the direction is reversed. Relevant for the display in the CMDB Explorer and for service trees.

### Positionable in rack

When set to **Yes**, objects of this type can be positioned on rack units in the [rack view](kategorien/s-enclosure.md). Prerequisite: The [form factor category](kategorien/formfactor.md) must be filled with the number of rack units. Disable this option for object types that do not belong in a rack (buildings, persons, software).
...
The default object image displayed in the information bar at the top (e.g. a server icon). Each object can receive an individual image via the [Object image](kategorien/image.md) category, but here you set the default for the entire type.
...
The technical identifier of the object type, e.g. `C__OBJTYPE__SERVER`. This constant is used in the [API](../i-doit-add-ons/api/index.md), in SQL reports and in [CSV import](../daten-konsolidieren/csv-datenimport/index.md). i-doit automatically generates a constant with a Unix timestamp -- for better readability, replace the timestamp with a descriptive name in uppercase, e.g. `C__OBJTYPE__NETWORK_PRINTER`.
...
Select a [template](../effizientes-dokumentieren/templates.md) that is automatically applied to new objects of this type. All fields from the template are pre-filled -- the user can override them if needed. Useful for pre-populating default values such as location, contact person or CMDB status.
...
The most important setting: here you determine which [categories](kategorien-und-attribute.md) are available for this object type. Activate the checkbox on the left to assign a category. The checkbox on the right determines whether the category appears on the **overview page** (only relevant if the overview page is enabled).
...
Detailed information can be found under [Assigning categories to object types](zurodnung-von-kategorien-zu-objekttypen.md).
...
If [JDisc import profiles](../daten-konsolidieren/jdisc/index.md) are configured, you can set a default profile for this object type here. It is preselected in the "JDisc Discovery" category.
