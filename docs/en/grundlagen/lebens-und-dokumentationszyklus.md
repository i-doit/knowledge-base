...
To map the entire life cycle of an object, the [attribute](../glossar.md) **CMDB status** in the [category](../glossar.md) **General** is used per [object](../glossar.md). When a new object is created, it receives **In operation** as its **CMDB status**, unless something else is explicitly specified.
...
-   Unique name or [language constant (for translation)](../administration/mehrsprachigkeit-und-uebersetzungen.md)
-   Unique constant (helpful for example when using the [API](../i-doit-add-ons/api/index.md))
-   Color (used in object lists, the **General** category, in the [**CMDB Explorer**](../auswertungen/cmdb-explorer/index.md) and in many other places)
...
-   **Template**:<br> An object can serve as a [template](../effizientes-dokumentieren/templates.md) for further objects.
-   **Change template**:<br> An object can serve as a change template for [mass changes](../effizientes-dokumentieren/massenaenderung.md).
...
1. Open the [object list](objekt-liste/index.md).
...
A similar functionality as for objects exists for some [list categories](../glossar.md) ("multi-value"). Through this, category entries can be archived, marked as deleted and purged.
...
To list all archived or deleted objects, it is best to create a [report](../auswertungen/report-manager.md) via the query editor.
...
The [i-doit console utility](../automatisierung-und-integration/cli/index.md) also offers you the option to irrevocably delete unwanted objects. Additional options are shown by the `--help` option.
