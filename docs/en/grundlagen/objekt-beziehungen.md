...
[Objects](struktur-it-dokumentation.md) in i-doit typically do not stand alone, but in relation to each other. Servers are located in racks, admins manage servers, an operating system with software runs on a server. This article explains the fundamental concepts of object relationships.
...
When a relationship between two objects is created, a third object of [type](struktur-it-dokumentation.md) **Relationships** is automatically created in the background. It receives the object title of the master object, followed by the description and the object title of the slave object.
...
The relationship object is necessary to store properties of the relationship as [attributes](struktur-it-dokumentation.md). These include:
...
The direction of the relationship -- i.e. which object is master, which is slave -- affects the [CMDB Explorer](../auswertungen/cmdb-explorer/index.md). In the tree view, object relationships are recursively built either upward or downward. In the [standard profile](../auswertungen/cmdb-explorer/profile-im-cmdb-explorer.md), master objects are positioned below the slave objects.

!!! info "Relationship objects are not [subject to licensing](../wartung-und-betrieb/lizenzierung.md)."

## Relationships category

Per object, relationships to other objects are collected in the [category](struktur-it-dokumentation.md) **Relationships**. This category is permanently assigned to every object type and can be accessed via the arrows icon.
...
The analysis module uses this weighting in combination with services and [network monitoring](../automatisierung-und-integration/network-monitoring/index.md) to make statements about which services are affected by outages.
