...
A freely selectable date on which i-doit should trigger a reminder — typically several weeks before contract end, so that enough time remains for a cancellation or renegotiation. Prerequisite: the [notifications](../../auswertungen/benachrichtigungen.md) must be configured.
