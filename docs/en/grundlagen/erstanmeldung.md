...
i-doit comes with several default users, each assigned to a group with predefined [permissions](../effizientes-dokumentieren/rechteverwaltung/index.md):
...
Every user in i-doit is an [object](struktur-it-dokumentation.md) of the type **Persons**. After the first login, you should urgently perform the following steps:

1. Set up an [LDAP connection](../benutzerauthentifizierung-und-verwaltung/ldap-verzeichnis/index.md) or create additional local users.
2. Change the credentials of the default users: Navigate to the [object type group](struktur-it-dokumentation.md) **Contacts**, then to the [object type](struktur-it-dokumentation.md) **Persons**, select the respective user and modify the category **Persons → Login**.
3. Alternatively, you can [archive](lebens-und-dokumentationszyklus.md) the default **Persons** objects -- this will disable their login.

!!! success "Local administrator"
    Even if an [LDAP directory server or an Active Directory (AD)](../benutzerauthentifizierung-und-verwaltung/ldap-verzeichnis/index.md) is being used, it is advisable to still create a local user with full permissions. If the external service becomes unavailable, you can still log in with the local user.
