#!/bin/bash
##
## i-doit automatisches Installations-Script für Debian 13 "Trixie"
##
## Verwendung:
##   ./idoit-install-debian13.sh
##
...
##   IDOIT_LICENSE_TOKEN=889bdc3c6a119da5f0a5d9c0db8747c6 ./idoit-install-debian13.sh
##
...
check_os() {
    ...
    [[ "$NAME" == "Debian GNU/Linux" && "$VERSION_ID" == "13" ]] \
        || err "Dieses Script läuft nur auf Debian 13 Trixie (gefunden: ${PRETTY_NAME})"
...
install_packages() {
    ...
    # CD-ROM Quelle entfernen falls vorhanden (bleibt nach Minimalinstallation übrig)
    sed -i '/^deb cdrom:/d' /etc/apt/sources.list 2>/dev/null || true
    find /etc/apt/sources.list.d/ -name "*.list" -exec sed -i '/^deb cdrom:/d' {} \; 2>/dev/null || true

    # Unterbrochene dpkg-Konfiguration abschließen falls nötig
    dpkg --configure -a 2>/dev/null || true

    apt-get -qq -y update
    # ca-certificates zuerst installieren damit HTTPS-Downloads funktionieren
    ...
    apt-get -qq -y install --no-install-recommends \
        ...
        memcached \
        unzip wget sudo moreutils \
        php-bcmath php-cli php-common php-curl php-gd \
        php-imagick php-json php-ldap php-mbstring \
        php-memcached php-mysql php-pgsql \
...
configure_apache() {
    ...
    # mod_php deaktivieren falls installiert, FPM-Module aktivieren
...
configure_mariadb() {
    ...
    # innodb_fast_shutdown vor Stop setzen
...
install_idoit() {
    ...
    log "Lade i-doit ${version} herunter: ${url}"
...
setup_idoit() {
    ...
    # config.inc.php Besitzer korrigieren
...
set_license() {
    ...
    # Token direkt in config.inc.php schreiben (umgeht den Online-Check von license-key)
    ...
    # Lizenz vom i-doit Lizenzserver importieren
    local import_out
    ...
    if echo "$import_out" | grep -q "OK" && ! echo "$import_out" | grep -q "FAIL"; then
        ...
        echo "         Token gesetzt — bitte im Admin Center unter Lizenzen manuell importieren:"
        echo "         http://$(ip route get 1 2>/dev/null | sed -n 's/.*src \([0-9.]*\).*/\1/p' | head -1)/?adminCenter"
    fi
}
